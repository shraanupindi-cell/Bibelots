...
import { findKnownConnections, findInferredConnections } from '../connections'
...
function aiLineStyle(confidence) {
  if (confidence === 3) return { w: 2.5, dash: undefined, op: 1.0 }
  return { w: 1.5, dash: '6 3', op: 0.85 }
}

// Simple, reliable node spreading: resolve overlaps, then clamp to canvas.
// Ring radius (= chronology) is respected because minDist / iteration counts are kept
// conservative — nodes only move as much as needed to stop overlapping.
function spreadNodes(rawPos, minDist, cx, cy, youR, W, H, margin, iters=150) {
  ...
  const minFromCentre = youR + minDist * 0.6
  for (let it = 0; it < iters; it++) {
    let moved = false
    for (const id of ids) {
      const p = pos[id], dx = p.x-cx, dy = p.y-cy, d = Math.sqrt(dx*dx+dy*dy) || 1
      if (d < minFromCentre) {
        pos[id] = { x: p.x + (dx/d)*(minFromCentre-d), y: p.y + (dy/d)*(minFromCentre-d) }
        moved = true
      }
    }
...
export default function Constellation({ trinkets, onReveal, onBack }) {
  ...
  // What actually renders: known connections + AI connections (deduped against known).
  // Hardcoded "inferred" rules are dropped entirely — AI is the sole inferred source.
  ...
  // Groq AI connections — CIDOC CRM grounded, field-by-field cross reference
  useEffect(() => {
...
TASK: Cross-reference every field of every object against every other object before drawing any connection. For each pair, systematically check:
...
A connection is valid if at least one field comparison yields a real, named, verifiable historical fact. A place implies its documented ruling power for that era (e.g. "Charminar, Hyderabad" + 1600s origin implies Qutub Shahi period; 1700s-1940s implies Nizam/Asaf Jahi period).

Use only these CIDOC CRM relationship types:
P9i (creation): shared production tradition, workshop, craft technique, or guild lineage.
P12i (event): shared historical period, dynasty, or event with overlapping dates.
P30i (custody transfer): moved through the same trade route, colonial system, or documented lineage.

REQUIREMENTS — every connection needs all three: (a) the CIDOC property, (b) a real named entity (specific dynasty/company/route/guild — not a vague category), (c) an approximate date range. If unsure the entity is real, skip the pair.
...
{"connections":[{"object1":"exact name","object2":"exact name","relation":"P12i","confidence":3,"label":"4-6 word label","detail":"1-2 sentences naming the dynasty/route/institution and date range."}]}`
    ...
    .then(data => {
      ...
      if (!match) { console.warn('No JSON in Groq response'); setAiLoading(false); return }
    ...
    .catch(err => { console.warn('Groq fetch error:', err.message); setAiLoading(false) })
  }, [trinkets.length])

  // Orbital drift clock
  useEffect(() => {
    const start = performance.now()
    const tick = (now) => { setOrbitAngle((now-start)/1000*(2*Math.PI/25)); orbitRef.current = requestAnimationFrame(tick) }
  ...
  // Entry animation
  useEffect(() => {
    setVisibleNodes([]); setVisibleConns([]); setNodeProgress({})
    const timers = []
  ...
  // Ring radius: oldest (smallest year) -> MAX_R (outermost). Newest (largest year) -> MIN_R (innermost).
  const rawPos = useMemo(() => {
    ...
    trinkets.forEach((t,i) => {
      ...
      p[t.id] = {
        ...
        y: Math.max(margin, Math.min(H-margin, cy+r*Math.sin(angle))),
      }
    })
    return p
  ...
  const ringRadii = useMemo(() => {
    const range = maxYear-minYear || 1, m = {}
    trinkets.forEach((t,i) => { m[t.id] = MAX_R - ((trinketYears[i]-minYear)/range)*(MAX_R-MIN_R) })
    return m
  }, [trinkets, trinketYears, minYear, maxYear])
  ...
  const minDist = useMemo(()=>(trinkets.length ? Math.max(...trinkets.map(t=>nodeR(t.name,t.id)))*2+20 : 50),[trinkets])
  ...
  const getDriftPos = (id, ap, idx) => {
    ...
    const phase = idx*2.399963, driftR = 10
    return { x: ap.x+Math.cos(orbitAngle+phase)*driftR, y: ap.y+Math.sin(orbitAngle+phase)*driftR }
  }
      ...
      {view==='map' ? (
        <div style={{position:'fixed',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'space-between',padding:'0.75rem 0.5rem'}}>
          ...
          <div style={{display:'flex',gap:'8px',marginBottom:'4px',flexWrap:'wrap',justifyContent:'center',alignItems:'center'}}>
            ...
            <button onClick={()=>setShowKnown(v=>!v)} style={{padding:'5px 14px',borderRadius:'99px',border:`1px solid ${showKnown?'#E8E0D0':'#444'}`,background:showKnown?'rgba(232,224,208,0.15)':'none',color:showKnown?'#E8E0D0':'#686860',fontFamily:'Inconsolata,monospace',fontSize:'10px',letterSpacing:'0.06em',cursor:'pointer'}}>
              {showKnown?'✓ ':''}Known
            </button>
            <button onClick={()=>setShowInferred(v=>!v)} style={{padding:'5px 14px',borderRadius:'99px',border:`1px solid ${showInferred?'#E8E0D0':'#444'}`,background:showInferred?'rgba(232,224,208,0.15)':'none',color:showInferred?'#E8E0D0':'#686860',fontFamily:'Inconsolata,monospace',fontSize:'10px',letterSpacing:'0.06em',cursor:'pointer'}}>
          ...
          {!aiLoading && aiConns.length>0 && <p style={{fontFamily:'Inconsolata,monospace',fontSize:'11px',color:'#6A9060',marginBottom:'3px',fontStyle:'italic'}}>{aiConns.length} provenance {aiConns.length===1?'link':'links'} traced from the archive</p>}
          ...
          {trinkets.length<3 ? (
            ...
            <svg viewBox={`0 0 ${W} ${H}`} style={{width:'auto',height:'auto',maxWidth:`${W}px`,maxHeight:'62vh',display:'block',margin:'0 auto'}}>
              ...
              {trinkets.map((t,i)=>{
                ...
                return <line key={t.id} x1={cx+(dx/d)*youR} y1={cy+(dy/d)*youR} x2={p.x-(dx/d)*nr} y2={p.y-(dy/d)*nr} stroke="#4A4A42" strokeWidth="1.2"/>
              })}
              ...
              {activeConns.map((c,i)=>{
                ...
                const ldx=p2raw.x-p1raw.x, ldy=p2raw.y-p1raw.y, llen=Math.sqrt(ldx*ldx+ldy*ldy)||1
                ...
                return (
                  <g key={i} onClick={()=>{setSelectedConn(isSel?null:c); if(showTip){setShowTip(false);localStorage.setItem('bib_tip','1')}}} style={{cursor:'pointer'}}>
                    ...
                    <line x1={p1.x} y1={p1.y} x2={p2.x} y2={p2.y}
                      ...
                      opacity={dimmed?0.15:isSel?1:c.ai?0.95:s.op}
              ...
              {trinkets.map((t,idx)=>{
                ...
                const nr = nodeR(t.name,t.id)
                const fill = getNodeFill(idx), tc = getTextFill(fill)
                ...
                return (
                  <g key={t.id}
                    onMouseEnter={()=>{
                      ...
                      const pulse = (now)=>{
                        const r = nr+((now-s0)/500)*36
                        setRipple({id:t.id,r})
                        if ((now-s0)<500) rippleRef.current=requestAnimationFrame(pulse)
          ...
          {trinkets.length>=3 && (
            <div style={{display:'flex',gap:'12px',flexWrap:'wrap',justifyContent:'center',marginTop:'6px'}}>
              {usedTypes.map(type=>{
                const s = type==='AI' ? {w:2,dash:'8 3',op:0.95} : (LS[type]||LS.historical)
                return (
          ...
          {selectedConn && (()=>{
            ...
            const cidocLabels = {P9i:'shared creation tradition', P12i:'shared historical period', P30i:'shared custody network'}
            return (
              <div style={{position:'absolute',bottom:'72px',background:'#141414',border:'0.5px solid #3A3A3A',borderRadius:'6px',padding:'12px 16px',maxWidth:'400px',textAlign:'center',maxHeight:'200px',overflowY:'auto',zIndex:10}}>
                ...
                {selectedConn.ai && <div style={{fontSize:'10px',color:'#6A8860',fontFamily:'Inconsolata,monospace',marginBottom:'5px',letterSpacing:'0.06em'}}>
                  {cidocLabels[selectedConn.type]||selectedConn.type} · confidence {selectedConn.confidence===3?'high':'medium'}
                </div>}
          ...
          {trinkets.length>=3 && (
            <div style={{display:'flex',gap:'10px',alignItems:'center',marginTop:'10px',flexWrap:'wrap',justifyContent:'center'}}>
              <button onClick={onBack} style={{padding:'10px 24px',borderRadius:'99px',border:'1px solid #A0A090',background:'rgba(232,224,208,0.08)',color:'#C8C4BC',fontFamily:'Inconsolata,monospace',fontSize:'12px',cursor:'pointer',letterSpacing:'0.06em'}}>
                Add More Objects
              </button>
              <button onClick={onReveal} style={{padding:'10px 36px',borderRadius:'99px',border:'1px solid #E8E0D0',background:'none',color:'#E8E0D0',fontFamily:'JacquardaBastarda9,cursive',fontSize:'20px',cursor:'pointer'}}
        ...
        <div style={{position:'fixed',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:'3rem',background:'#1E1E1E'}}>
          <button onClick={()=>setView('map')} style={{position:'absolute',top:'24px',left:'24px',background:'rgba(232,224,208,0.1)',border:'1px solid #E8E0D0',borderRadius:'99px',padding:'7px 18px',color:'#E8E0D0',fontFamily:'Inconsolata,monospace',fontSize:'12px',cursor:'pointer',letterSpacing:'0.06em'}}>Back to Map</button>
          ...
          <button onClick={onReveal} style={{marginTop:'2rem',padding:'10px 36px',borderRadius:'99px',border:'1px solid #E8E0D0',background:'none',color:'#E8E0D0',fontFamily:'JacquardaBastarda9,cursive',fontSize:'20px',cursor:'pointer'}}>Reveal Archetype</button>
        </div>
      )}
    </div>
  )
